...
return (
    <main className='flex flex-row-reverse h-full bg-cover w-full bg-no-repeat' style={{backgroundImage: `url(${Cover})`}}>
        <div className='bg-white p-10 pt-4 rounded-l-3xl w-[50%] flex justify-center'>
            <form onSubmit={handleSignup} className="m-10 mt-0 flex flex-col w-full">
                <h1 className="text-lg font-600 text-center mb-3">Sign-up</h1>
                ...
                onChange={(e) => setForm({ ...form, email: e.target.value })} className="border p-3 rounded-md text-sm bg-gray-200"/>
                ...
                onChange={(e) => setForm({ ...form, company_name: e.target.value })} className="border p-3 rounded-md text-sm bg-gray-200"/>

                <label className="text-gray-500 mt-4 text-sm" htmlFor="job title">Job Title</label>
                <select value={form.job_title} onChange={(e) => setForm({...form, job_title: e.target.value})} required className="border p-3 rounded-md bg-gray-200 text-gray-500 text-sm">
                ...
                <select value={form.team_size} onChange={(e) => setForm({...form, team_size: e.target.value})} required className="border p-3 rounded-md bg-gray-200 text-gray-500 text-sm">
        ...
        <button className='bg-[#2161F6] text-[16px] mt-6 text-white p-2 rounded-md w-[200px] mx-auto'>Create Account</button>
        <span className='text-gray-400 text-sm text-center mt-2'>Already have an account? 
            <Link to='/login' className='ms-1 text-[#00B5D8]'>
                Log in
            </Link></span>
    </form>
        </div>
